...
export default function NodeInspector({
  ...
  const title = useMemo(() => {
    ...
    return t('workflow2.legacyInspector.titleWithKind', {
      name: node.data.name || t('workflow2.node.unnamed'),
      kind: node.data.kind,
    });
  }, [node, t]);
  ...
  const outputSelectOptions = useMemo(() => {
    ...
    add('', t('workflow2.legacyInspector.outputSelect.auto'));
    add('data', t('workflow2.legacyInspector.outputSelect.dataWhole'));
    add('input', t('workflow2.legacyInspector.outputSelect.inputWhole'));
    ...
    for (const m of incomingMappings) {
      ...
      for (const o of hintOutputs) {
        const path = `${base}.${o}`;
        add(path, t('workflow2.legacyInspector.outputSelect.whenObject', { path, base }));
      }
    }
  ...
  const referenceWarnings = useMemo(() => {
    ...
    for (const expr of tokens) {
      ...
      if (root && !hintSet.has(root)) warnings.push(t('workflow2.legacyInspector.validation.maybeMissingRef', { expr }));
    }
    return warnings;
  }, [cfg, node, t, variableHints]);
  ...
  const nodeValidation = useMemo(() => {
    ...
    if (!node.data.name?.trim()) errors.push(t('workflow2.legacyInspector.validation.nodeNameRequired'));

    if (kind === 'llm') {
      const temperature = Number(cfg.temperature ?? 0.7);
      const mt = Number(cfg.max_tokens ?? 1000);
      if (!Number.isFinite(temperature) || temperature < 0 || temperature > 2)
        errors.push(t('workflow2.legacyInspector.validation.temperatureRange'));
      if (!Number.isFinite(mt) || mt < 1) errors.push(t('workflow2.legacyInspector.validation.maxTokensPositive'));
      if (!cfg.system_prompt && incomingEdges.length === 0) warnings.push(t('workflow2.legacyInspector.validation.llmNoUpstream'));
    }

    if (kind === 'rag_retriever') {
      if (!cfg.knowledge_base) errors.push(t('workflow2.legacyInspector.validation.selectKnowledgeBase'));
      const k = Number(cfg.top_k ?? 5);
      if (!Number.isFinite(k) || k < 1 || k > 50) errors.push(t('workflow2.legacyInspector.validation.topKRange'));
    }

    if (kind === 'condition') {
      if (!String(cfg.field_path || '').trim()) errors.push(t('workflow2.legacyInspector.validation.fieldPathRequired'));
      const ct = String(cfg.condition_type || 'equals');
      if (!ct) errors.push(t('workflow2.legacyInspector.validation.conditionTypeRequired'));
      if (ct !== 'truthy' && String(cfg.condition_value ?? '').trim() === '')
        warnings.push(t('workflow2.legacyInspector.validation.conditionValueEmpty'));
    }

    if (kind === 'code_executor') {
      const code = String(cfg.code || '');
      if (!code.trim()) errors.push(t('workflow2.legacyInspector.validation.codeRequired'));
      if (!/\bresult\s*=/.test(code)) warnings.push(t('workflow2.legacyInspector.validation.codeResultHint'));
      if (/\bimport\b/.test(code)) warnings.push(t('workflow2.legacyInspector.validation.sandboxNoImport'));
      if (/__\w+__/.test(code)) warnings.push(t('workflow2.legacyInspector.validation.sandboxNoDunder'));
      ...
      if (!Number.isFinite(timeoutSec) || timeoutSec < 0.1 || timeoutSec > 30) errors.push(t('workflow2.legacyInspector.validation.timeoutSecRange'));
      if (!Number.isFinite(memMb) || memMb < 16 || memMb > 4096) errors.push(t('workflow2.legacyInspector.validation.maxMemoryRange'));
      if (!Number.isFinite(stdoutChars) || stdoutChars < 1000 || stdoutChars > 200000) errors.push(t('workflow2.legacyInspector.validation.maxStdoutRange'));
      if (!Number.isFinite(maxInBytes) || maxInBytes < 10000 || maxInBytes > 50_000_000) errors.push(t('workflow2.legacyInspector.validation.maxInputBytesRange'));
      if (!Number.isFinite(maxOutBytes) || maxOutBytes < 10000 || maxOutBytes > 50_000_000) errors.push(t('workflow2.legacyInspector.validation.maxResultBytesRange'));
    }

    if (kind === 'output') {
      const f = String(cfg.format || 'json');
      if (!['json', 'text', 'markdown'].includes(f)) errors.push(t('workflow2.legacyInspector.validation.outputFormatUnsupported'));
      const tpl = String(cfg.template || '');
      const sel = String(cfg.select_path || cfg.select || '');
      if (sel && tpl) warnings.push(t('workflow2.legacyInspector.validation.outputSelectOverridden'));
      if (tpl && tpl.trim() === '') warnings.push(t('workflow2.legacyInspector.validation.outputTemplateBlank'));
      if (tpl && !tpl.includes('{{') && !tpl.includes('{')) warnings.push(t('workflow2.legacyInspector.validation.outputTemplateNotConfigured'));
    }

    if (kind === 'http_request') {
      const url = String(cfg.url || '').trim();
      if (!url) errors.push(t('workflow2.legacyInspector.validation.urlRequired'));
      const m = String(cfg.method || 'GET').toUpperCase();
      if (!['GET', 'POST', 'PUT', 'PATCH', 'DELETE'].includes(m))
        errors.push(t('workflow2.legacyInspector.validation.methodUnsupported'));
      const timeout = Number(cfg.timeout ?? 30);
      if (!Number.isFinite(timeout) || timeout <= 0) errors.push(t('workflow2.legacyInspector.validation.timeoutPositive'));
      if (cfg.headers != null && typeof cfg.headers !== 'object') errors.push(t('workflow2.legacyInspector.validation.headersMustBeObject'));
      if (cfg.params != null && typeof cfg.params !== 'object') errors.push(t('workflow2.legacyInspector.validation.paramsMustBeObject'));
    }

    // overrides (engine supports it)
    const overrides = cfg.overrides;
    if (overrides != null && typeof overrides !== 'object') errors.push(t('workflow2.legacyInspector.validation.overridesMustBeObject'));

    return { errors, warnings: [...warnings, ...referenceWarnings] };
  }, [cfg, incomingEdges.length, kind, node, referenceWarnings, t]);
  ...
  const quickFill = (preset: 'rag_to_llm' | 'docs_to_prompt') => {
    if (preset === 'rag_to_llm') {
      ...
      updateConfig(
        'system_prompt',
        t('workflow2.legacyInspector.quickFill.ragToLlmSystemPrompt')
      );
      return;
    }
    if (preset === 'docs_to_prompt') {
      setOverride('prompt', t('workflow2.legacyInspector.quickFill.docsToPrompt'));
      return;
    }
  };
  ...
  const parseJsonObject = (raw: string, setErr: (msg: string | null) => void): any => {
    ...
    try {
      ...
      setErr(t('workflow2.legacyInspector.jsonErrors.mustBeObject'));
      return null;
    } catch (e: any) {
      setErr(e?.message || t('workflow2.legacyInspector.jsonErrors.parseFailed'));
      return null;
    }
  };
  ...
  const pickerGroups = useMemo(() => {
    ...
    groups.push({ label: t('workflow2.legacyInspector.templatePicker.groups.runtime'), items: globalTemplateHints.slice() });
    ...
    return groups;
  }, [globalTemplateHints, incomingMappings, t]);
    ...
    return {
      ...
      InputProps: {
        endAdornment: (
          ...
          >
            {t('workflow2.legacyInspector.templatePicker.insert')}
          </Button>
        ),
      },
    };
  ...
  const renderField = (f: NodeFieldSchema) => {
    ...
    const fieldHelperText = f.helperTextKey ? t(f.helperTextKey) : f.helperText;
    const fieldPlaceholder = f.placeholderKey ? t(f.placeholderKey) : f.placeholder;
    const optionLabel = (o: SchemaOption) => (o.labelKey ? t(o.labelKey) : (o.label ?? String(o.value)));
    if (f.type === 'select') {
      ...
      if (kind === 'llm' && f.key === 'prompt_key') {
        ...
        return (
          <FormControl key={f.key} fullWidth size="small">
            ...
            <Select value={value} label={fieldLabel} onChange={(e) => updateConfig(f.key, e.target.value)}>
              ...
              <ListSubheader>{t('workflow2.legacyInspector.promptKey.sections.runtime')}</ListSubheader>
              {runtimeOpts.map((o) => (
              ...
              {connectedOpts.length > 0 && <ListSubheader>{t('workflow2.legacyInspector.promptKey.sections.fromEdges')}</ListSubheader>}
              {connectedOpts.map((o) => (
              ...
              {otherOpts.length > 0 && <ListSubheader>{t('workflow2.legacyInspector.promptKey.sections.other')}</ListSubheader>}
              {otherOpts.map((o) => (
            ...
            <FormHelperText>
              {connectedOpts.length
                ? t('workflow2.legacyInspector.promptKey.hintConnected')
                : t('workflow2.legacyInspector.promptKey.hintDisconnected')}
            </FormHelperText>
          </FormControl>
        );
      }
      ...
      return (
        <FormControl key={f.key} fullWidth size="small">
          <InputLabel>{fieldLabel}</InputLabel>
          <Select value={String(val ?? '')} label={fieldLabel} onChange={(e) => updateConfig(f.key, e.target.value)}>
            {opts.map((o) => (
              <MenuItem key={`${f.key}_${o.value}`} value={o.value}>
                {optionLabel(o)}
              </MenuItem>
    ...
    if (f.type === 'json_object') {
      ...
      return (
        <TextField
          ...
          error={!!err}
          helperText={err || fieldHelperText}
          multiline
          minRows={3}
        />
      );
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2, height: '100%', overflow: 'auto' }}>
            ...
            <Box sx={{ px: 1, py: 0.75, display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
              <Typography variant="caption" color="text.secondary">
                {tplMode === 'autocomplete'
                  ? (
                      <>
                        {t('workflow2.legacyInspector.templatePicker.autocompleteHint')}
                      </>
                    )
                  : t('workflow2.legacyInspector.templatePicker.pickerHint')}
              </Typography>
            ...
            {tplMode === 'autocomplete' ? (
              <MenuList dense sx={{ maxHeight: 260, overflow: 'auto' }}>
                ...
                {tplItems.length === 0 && (
                  <MenuItem disabled>
                    <Typography variant="caption" color="text.secondary">
                      {t('workflow2.legacyInspector.templatePicker.empty')}
                    </Typography>
                  </MenuItem>
                )}
              </MenuList>
                  ...
                  <Box key={g.label} sx={{ px: 0.5, py: 0.5 }}>
                    ...
                    <MenuList dense disablePadding>
                      ...
                      {g.items.length === 0 && (
                        <MenuItem disabled>
                          <Typography variant="caption" color="text.secondary">
                            {t('workflow2.legacyInspector.templatePicker.none')}
                          </Typography>
                        </MenuItem>
                      )}
                    </MenuList>
      ...
      <Tabs
        ...
        sx={{ mb: 2 }}
      >
        <Tab value="config" label={t('workflow2.legacyInspector.tabs.config')} />
        <Tab value="io" label={t('workflow2.legacyInspector.tabs.io')} />
        <Tab value="advanced" label={t('workflow2.legacyInspector.tabs.advanced')} />
      </Tabs>
      ...
      {activeTab === 'io' && (
        <>
          <Accordion sx={{ mb: 2 }} defaultExpanded={kind === 'output'}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography sx={{ fontWeight: 800 }}>
                {t('workflow2.legacyInspector.io.inputsTitle', { count: nodeInputs.length })}
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Typography variant="caption" color="text.secondary">
                {t('workflow2.legacyInspector.io.inputsHint')}
              </Typography>
              <Box sx={{ mt: 1, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {nodeInputs.length ? nodeInputs.map((k) => <Chip key={k} size="small" label={k} />) : (
                  <Typography variant="caption" color="text.secondary">
                    {t('workflow2.legacyInspector.io.noInputs')}
                  </Typography>
                )}
              </Box>
            </AccordionDetails>
          ...
          <Accordion sx={{ mb: 2 }} defaultExpanded={kind === 'output'}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography sx={{ fontWeight: 800 }}>
                {t('workflow2.legacyInspector.io.outputsTitle', { count: nodeOutputs.length })}
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Typography variant="caption" color="text.secondary">
                {t('workflow2.legacyInspector.io.outputsHint')}
              </Typography>
              <Box sx={{ mt: 1, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {nodeOutputs.map((k) => (
          ...
          <Accordion sx={{ mb: 2 }} defaultExpanded={false}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography sx={{ fontWeight: 800 }}>
                {t('workflow2.legacyInspector.io.outgoingTitle', { count: outgoingMappings.length })}
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Typography variant="caption" color="text.secondary">
                {t('workflow2.legacyInspector.io.outgoingHint')}
              </Typography>
              <Box sx={{ mt: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                {outgoingMappings.length === 0 && (
                  <Typography variant="caption" color="text.secondary">
                    {t('workflow2.legacyInspector.io.noOutgoing')}
                  </Typography>
                )}
              </Box>
            </AccordionDetails>
          ...
          {(kind === 'llm' || kind === 'rag_retriever' || kind === 'output') && (
            <Accordion sx={{ mb: 2 }} defaultExpanded={incomingMappings.length > 0}>
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Typography sx={{ fontWeight: 800 }}>
                  {kind === 'rag_retriever'
                    ? t('workflow2.legacyInspector.bindings.queryTitle')
                    : t('workflow2.legacyInspector.bindings.inputTitle')}
                  {incomingMappings.length ? t('workflow2.legacyInspector.bindings.countSuffix', { count: incomingMappings.length }) : ''}
                </Typography>
              </AccordionSummary>
              <AccordionDetails>
                {kind === 'llm' ? (
                  <Typography variant="caption" color="text.secondary">
                    {t('workflow2.legacyInspector.bindings.llmHint')}
                  </Typography>
                ) : kind === 'rag_retriever' ? (
                  <Typography variant="caption" color="text.secondary">
                    {t('workflow2.legacyInspector.bindings.ragHint')}
                  </Typography>
                ) : (
                  <Typography variant="caption" color="text.secondary">
                    {t('workflow2.legacyInspector.bindings.outputHint')}
                  </Typography>
                )}

                {incomingMappings.length === 0 ? (
                  <Alert severity="info" sx={{ mt: 1.25 }}>
                    {t('workflow2.legacyInspector.bindings.connectUpstreamFirst')}
                  </Alert>
                ) : (
                        ...
                        >
                          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}>
                            <Typography variant="caption" sx={{ fontWeight: 700 }}>
                              {m.srcName} → {node.data.name || (kind === 'llm' ? t('workflow2.kindLabels.llm') : kind === 'rag_retriever' ? t('workflow2.kindLabels.rag_retriever') : t('workflow2.kindLabels.output'))}
                            </Typography>
                            {showFix && (
                              <Button
                                size="small"
                              ...
                              >
                                {t('workflow2.legacyInspector.bindings.applyRecommended')}
                              </Button>
                            )}
                          </Box>
                          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>
                            <FormControl size="small" fullWidth disabled={!canEdit}>
                              <InputLabel>{t('workflow2.legacyInspector.bindings.fields.sourceOutput')}</InputLabel>
                              <Select
                                label={t('workflow2.legacyInspector.bindings.fields.sourceOutput')}
                            ...
                            <FormControl size="small" fullWidth disabled={!canEdit}>
                              <InputLabel>
                                {kind === 'llm'
                                  ? t('workflow2.legacyInspector.bindings.fields.mapToLlm')
                                  : kind === 'rag_retriever'
                                    ? t('workflow2.legacyInspector.bindings.fields.mapToRag')
                                    : t('workflow2.legacyInspector.bindings.fields.mapToOutput')}
                              </InputLabel>
                              <Select
                                label={
                                  kind === 'llm'
                          ...
                          <Typography variant="caption" color="text.secondary">
                            {t('workflow2.legacyInspector.bindings.recommendedPrefix')} <code>{recSo}</code> → <code>{recTi}</code>
                          </Typography>
                        </Paper>
                      );
                    })}
                ...
                {kind === 'llm' && (
                  <Box sx={{ mt: 1.25, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    <Button variant="outlined" size="small" onClick={() => quickFill('rag_to_llm')}>
                      {t('workflow2.legacyInspector.quickFill.actions.ragToLlmSystemPrompt')}
                    </Button>
                    <Button variant="outlined" size="small" onClick={() => quickFill('docs_to_prompt')}>
                      {t('workflow2.legacyInspector.quickFill.actions.composePromptOverrides')}
                    </Button>
                  </Box>
                )}
              </AccordionDetails>
          ...
          <Accordion sx={{ mb: 2 }} defaultExpanded={false}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography sx={{ fontWeight: 800 }}>
                {t('workflow2.legacyInspector.variableHints.title', { runtime: globalTemplateHints.length, edges: variableHints.length })}
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Typography variant="caption" color="text.secondary">
                {t('workflow2.legacyInspector.variableHints.hintAutocomplete')}
              </Typography>

              <Typography variant="caption" color="text.secondary" sx={{ mt: 1.25, display: 'block' }}>
                {t('workflow2.legacyInspector.variableHints.sections.runtimeInput')}
              </Typography>
              ...
              <Typography variant="caption" color="text.secondary" sx={{ mt: 1.25, display: 'block' }}>
                {t('workflow2.legacyInspector.variableHints.sections.runtimeContext')}
              </Typography>
              <Box sx={{ mt: 0.5, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
              ...
              <Typography variant="caption" color="text.secondary" sx={{ mt: 1.25, display: 'block' }}>
                {t('workflow2.legacyInspector.variableHints.sections.fromEdges')}
              </Typography>
              <Box sx={{ mt: 0.5, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {variableHints.map((k) => (
                  <Chip
                ...
                {variableHints.length === 0 && (
                  <Typography variant="caption" color="text.secondary">
                    {t('workflow2.legacyInspector.variableHints.emptyFromEdges')}
                  </Typography>
                )}
              </Box>
            </AccordionDetails>
        ...
        <>
          {schema.length === 0 && (
            <Alert severity="info" sx={{ mb: 1.5 }}>
              {t('workflow2.inspector.noConfig')}
            </Alert>
          )}
            ...
            return (
              <Box key={g} sx={{ mb: 2 }}>
                ...
                {isSandbox ? (
                  <Accordion>
                    <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                      <Typography sx={{ fontWeight: 700 }}>{t('workflow2.legacyInspector.sandbox.title')}</Typography>
                    </AccordionSummary>
                    <AccordionDetails>
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                          ...
                          >
                            {t('workflow2.legacyInspector.sandbox.resetDefaults')}
                          </Button>
                        </Box>
                      </Box>
                    </AccordionDetails>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                    ...
                    {kind === 'code_executor' && g === 'code' && (
                      <Typography variant="caption" color="text.secondary">
                        {t('workflow2.legacyInspector.codeExecutor.convention')}
                      </Typography>
                    )}
                    ...
                    {kind === 'condition' && g === 'basic' && (
                      <>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {t('workflow2.legacyInspector.branches.hint')}
                        </Typography>
                      </>
                    )}

                    {kind === 'output' && g === 'template' && (
                      <>
                        <FormControl size="small" fullWidth>
                          <InputLabel>{t('workflow2.legacyInspector.output.selectPathOptional')}</InputLabel>
                          <Select
                            label={t('workflow2.legacyInspector.output.selectPathOptional')}
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {t('workflow2.legacyInspector.output.priorityHint')}
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
                          {t('workflow2.legacyInspector.output.templateHint')}
                        </Typography>
                      </>
                    )}
                  </Box>
          ...
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography sx={{ fontWeight: 800 }}>{t('workflow2.legacyInspector.ioPreview.title')}</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Typography variant="caption" color="text.secondary">
                {t('workflow2.legacyInspector.ioPreview.hint')}
              </Typography>

              <Typography variant="caption" color="text.secondary" sx={{ mt: 1.25, display: 'block' }}>
                {t('workflow2.legacyInspector.ioPreview.inputsLabel', { count: nodeInputs.length })}
              </Typography>
              <Box sx={{ mt: 0.5, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {nodeInputs.map((k) => (
                ...
                {nodeInputs.length === 0 && (
                  <Typography variant="caption" color="text.secondary">
                    {t('workflow2.legacyInspector.common.none')}
                  </Typography>
                )}
              </Box>

              <Typography variant="caption" color="text.secondary" sx={{ mt: 1.25, display: 'block' }}>
                {t('workflow2.legacyInspector.ioPreview.outputsLabel', { count: nodeOutputs.length })}
              </Typography>
              <Box sx={{ mt: 0.5, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {nodeOutputs.map((k) => (
              ...
              <Box sx={{ mt: 1.25, display: 'flex', justifyContent: 'flex-end' }}>
                <Button size="small" variant="outlined" onClick={() => setActiveTab('io')}>
                  {t('workflow2.legacyInspector.ioPreview.goConfigure')}
                </Button>
              </Box>
            </AccordionDetails>
          </Accordion>
      ...
      {activeTab === 'advanced' && (
        <>
          <Accordion sx={{ mt: 0 }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography sx={{ fontWeight: 700 }}>{t('workflow2.legacyInspector.advanced.overridesTitle')}</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                {t('workflow2.legacyInspector.advanced.overridesHint')}
              </Typography>
              {(() => {
                const overrides =
                ...
                return (
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    ...
                    >
                      {t('workflow2.legacyInspector.advanced.addItem')}
                    </Button>
                  </Box>
                );
              })()}
          ...
          <Accordion sx={{ mt: 2 }} onChange={(_, expanded) => { if (expanded) setRawConfig(JSON.stringify(node.data.config || {}, null, 2)); }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography sx={{ fontWeight: 700 }}>{t('workflow2.legacyInspector.advanced.editJsonTitle')}</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <TextField
                fullWidth
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
                ...
                >
                  {t('workflow2.legacyInspector.advanced.applyJson')}
                </Button>
              </Box>
            </AccordionDetails>
          </Accordion>
