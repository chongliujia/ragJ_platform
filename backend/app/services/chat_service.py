...
class ChatService:
    ...
    async def chat(self, request: ChatRequest) -> ChatResponse:
        ...
        try:
            llm_response = await llm_service.chat(message=request.message, model=request.model)
    ...
    async def rag_chat(
        ...
        try:
            ...
            embedding_response = await llm_service.get_embeddings(texts=[request.message])
            if not embedding_response.get("success") or not embedding_response.get("embeddings", []):
                logger.warning(
            ...
            if not context:
                logger.warning("No relevant context found after reranking. Falling back to standard chat.")
                return await self.chat(ChatRequest(message=request.message, model=request.model, chat_id=request.chat_id))

            prompt = self._construct_rag_prompt(request.message, context)
            logger.info("Generating final response from LLM with reranked RAG context...")
...
"""
    ...
    async def stream_chat(self, request: ChatRequest) -> AsyncGenerator[str, None]:
        ...
        async for chunk in llm_service.stream_chat(message=request.message, model=request.model):
