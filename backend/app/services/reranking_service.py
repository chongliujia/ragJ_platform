...
class BGEReranker(BaseReranker):
    ...
    def _get_config(self):
        """动态获取配置"""
        try:
            from app.services.model_config_service import model_config_service, ModelType, ProviderType
            ...
            if rerank_config and rerank_config.provider == ProviderType.SILICONFLOW:
                return rerank_config.api_base or 'https://api.siliconflow.cn/v1', rerank_config.api_key
            
            # 回退到环境变量或默认值
            api_url = getattr(settings, 'SILICONFLOW_API_URL', 'https://api.siliconflow.cn/v1')
    ...
    async def rerank(
        ...
        try:
            ...
            if not api_key:
                logger.warning("BGE reranking API key not configured, falling back to no reranking")
            ...
            if response.status_code == 200:
                ...
                logger.error(f"BGE reranking failed: {response.status_code} - {response.text}")
                # 回退到原始排序
                return await NoReranker().rerank(query, documents, top_k)
...
class CohereReranker(BaseReranker):
    ...
    async def rerank(
        ...
        if not api_key:
            logger.warning("Cohere API key not configured, falling back to no reranking")
            return await NoReranker().rerank(query, documents, top_k)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Cohere reranking failed: {response.status_code} - {response.text}")
                return await NoReranker().rerank(query, documents, top_k)
...
class QwenReranker(BaseReranker):
    ...
    async def rerank(
        ...
        try:
            ...
            # 使用现有的嵌入服务计算相似度
            embedding_response = await llm_service.get_embeddings(texts=[query] + doc_texts)
...
class RerankingService:
    ...
    async def rerank_documents(
        ...
        if not reranker:
            logger.warning(f"Unknown reranking provider: {provider}, using no reranking")
        ...
        try:
            ...
            # 回退到无重排
            return await self.rerankers[RerankingProvider.NONE].rerank(query, documents, top_k)
    ...
    def get_available_providers(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "available": hasattr(settings, 'COHERE_API_KEY') and bool(settings.COHERE_API_KEY)
            },
            {
