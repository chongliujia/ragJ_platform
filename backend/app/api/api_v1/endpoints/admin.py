...
async def list_all_tenants(
    ...
    for tenant in tenants:
        ...
        current_knowledge_bases = db.query(KnowledgeBase).filter(KnowledgeBase.tenant_id == tenant.id).count()
        current_documents = db.query(Document).join(KnowledgeBase).filter(KnowledgeBase.tenant_id == tenant.id).count()
...
async def list_all_users(
    ...
    for user in users:
        # 统计用户的知识库和文档数量
        knowledge_bases_count = db.query(KnowledgeBase).filter(KnowledgeBase.owner_id == user.id).count()
...
async def list_all_knowledge_bases(
    ...
    """获取所有知识库列表"""
    query = db.query(KnowledgeBase).join(User, KnowledgeBase.owner_id == User.id).join(Tenant, KnowledgeBase.tenant_id == Tenant.id)
...
async def list_all_documents(
    ...
    """获取所有文档列表"""
    query = db.query(Document).join(KnowledgeBase, Document.knowledge_base_id == KnowledgeBase.id).join(User, Document.uploaded_by == User.id).join(Tenant, KnowledgeBase.tenant_id == Tenant.id)
...
async def update_role_permissions(
    ...
    # 获取权限ID映射
    permissions = db.query(Permission).filter(Permission.name.in_(request.permission_names)).all()
...
async def force_delete_user(
    ...
    # 2. 删除用户拥有的知识库和文档
    # 首先删除知识库中的文档
    user_knowledge_bases = db.query(KnowledgeBase).filter(KnowledgeBase.owner_id == user_id).all()
...
async def delete_tenant(
    ...
    # 删除租户下的所有文档
    documents = db.query(Document).join(KnowledgeBase).filter(KnowledgeBase.tenant_id == tenant_id).all()
    for doc in documents:
        db.delete(doc)
    ...
    # 删除租户下的所有用户配置
    user_configs = db.query(UserConfig).join(User).filter(User.tenant_id == tenant_id).all()
