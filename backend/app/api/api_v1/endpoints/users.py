...
from app.core.dependencies import get_current_active_user, require_super_admin, require_admin
...
async def get_user_config(
    ...
    return UserConfigResponse(
        ...
        updated_at=config.updated_at.isoformat() if config.updated_at else config.created_at.isoformat()
    )
...
async def update_user(
    ...
    if current_user.role != "super_admin":
        ...
        if user_update.role and user_update.role != user.role:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Permission denied: cannot change user role"
            )
...
async def delete_user(
    ...
    if current_user.role != "super_admin":
        ...
        if user.role in ["super_admin", "admin"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Permission denied: cannot delete admin users"
            )
