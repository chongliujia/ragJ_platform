...
async def test_llm_connectivity(
    ...
    try:
        ...
        if summary.get("all_connected"):
            ...
            logger.warning(
                "Some LLM providers failed to connect",
        ...
        raise HTTPException(
            status_code=500, detail=f"Connectivity test failed: {str(e)}"
        )


@router.post("/llm/chat")
async def test_llm_chat(
    request: LLMTestRequest, llm_service: LLMService = Depends(get_llm_service)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if result.get("success"):
            ...
            logger.error(
                "LLM chat test failed", model=request.model, error=result.get("error")
            )
            return {
                "status": "failed",
                "request": {"message": request.message, "model": request.model},
...
async def get_test_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "default_parameters": {"temperature": 0.7, "max_tokens": 1000, "top_p": 0.8},
        "api_endpoints": {
