...
def cleanup():
    ...
    try:
        ...
        if response.status_code == 204:
            ...
            print_result(
                True, f"Knowledge base '{KB_NAME}' did not exist, nothing to clean up."
            )
        else:
            print_result(
                False,
                f"Cleanup failed with status {response.status_code}: {response.text}",
            )
...
def run_test():
    ...
    try:
        ...
        if response.status_code != 200:
            print_result(
                False,
                f"Chat request failed. Status: {response.status_code}, Body: {response.text}",
            )
            return
        ...
        if EXPECTED_CONTEXT_IN_ANSWER.lower() in ai_message.lower():
            print_result(
                True,
                f"The response contains the expected information: '{EXPECTED_CONTEXT_IN_ANSWER}'.",
            )
        else:
            print_result(
                False,
                f"The response DID NOT contain the expected information: '{EXPECTED_CONTEXT_IN_ANSWER}'.",
            )
