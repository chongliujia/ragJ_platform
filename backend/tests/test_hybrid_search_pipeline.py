...
def run_test():
    ...
    try:
        ...
        if response.status_code != 201:
            ...
            return False
        print_result(True, f"Knowledge base '{KB_NAME}' created in Milvus and Elasticsearch.")
        ...
        if response.status_code != 200:
            print_result(False, f"Chat API call failed. Status: {response.status_code}, Body: {response.text}")
            return False
        ...
        if all(keyword in answer for keyword in EXPECTED_ANSWER_KEYWORDS):
            ...
            print_result(False, f"Answer does not contain all expected keywords: {EXPECTED_ANSWER_KEYWORDS}")
            return False
        ...
        response = requests.delete(
            f"{BASE_URL}/knowledge-bases/{KB_NAME}",
            timeout=30
        )
        if response.status_code == 204:
            print_result(True, "Cleanup successful.")
        else:
            print_result(False, f"Cleanup failed. Status: {response.status_code}, Body: {response.text}")
