...
graph TD
    ...
    Rust_Document_Processor -->|Storage| Document_Storage(S3/SeaweedFS)
...
*   **Rust 向量存储服务 (Rust Vector Store Service):**
    ...
    *   **技术栈:** Rust, `milvus-client`/`pgvector` (根据选择的向量数据库)。
*   **LLM 服务 (LLM Service):** 独立部署的大型语言模型服务，可以通过 API 调用。可以是 OpenAI API, Hugging Face Inference API, 或自部署的 LLM。
*   **文档存储 (Document Storage):** 存储原始文档文件。可以使用 S3 兼容的对象存储服务（如 SeaweedFS, AWS S3）。
*   **向量数据库 (Vector Database):** 存储文本块的向量嵌入，支持高效的相似性搜索。例如 Milvus, Weaviate, 或 Pgvector (如果使用 PostgreSQL)。
...
*   **对象存储:** SeaweedFS/AWS S3
*   **向量数据库:** Milvus/Weaviate (根据具体需求和性能考量选择)
*   **关系型数据库:** PostgreSQL
